...
pub async fn run(
    ...
    let tick_task = tokio::spawn(async move {
        while let Some(tick) = stream.next().await {
            let _res = deribit_price_tick_received(&price_stream_publisher, tick).await;
        }
    });
...
async fn deribit_price_tick_received(
    publisher: &memory::Publisher<PriceStreamPayload>,
    tick: DeribitPriceTick,
) -> Result<(), PriceFeedError> {
    if let Ok(payload) = PriceStreamPayload::try_from(tick) {
        publisher
            .throttle_publish("DERIBIT_PRICE_TICK", payload)
            .await?;
    }
    Ok(())
}
