...
ssh ${ADDITIONAL_SSH_OPTS} ${DOCKER_HOST_USER}@${DOCKER_HOST_IP} \
  "cd ${REPO_PATH}; OKEX_API_KEY=${OKEX_API_KEY} OKEX_PASSPHRASE=${OKEX_PASSPHRASE} OKEX_SECRET_KEY=${OKEX_SECRET_KEY} docker compose -f docker-compose.yml up integration-tests"
